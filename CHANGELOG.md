...
### Added
- **Recipient portal at `/portal`** — recipients can now sign in with their email and see every active project (`IN_REVIEW` or `APPROVED`) they're listed on across all clients, in a single dashboard. Sign-in uses a one-time magic link (5-minute, single-use, IP+UA-bound, Redis-backed) sent to the recipient's email; the portal session JWT (`type: 'portal'`, signed with the existing `SHARE_TOKEN_SECRET`) lives in `sessionStorage` for the same TTL as the existing client session setting and supports server-side denylist on logout. Clicking "Open" on a project opens `/share/<slug>` in a new tab and exchanges the portal session for a regular share token via `/api/share/[token]/portal-claim`, bypassing the per-project password/OTP gate (server re-checks recipient membership; security event logged on mismatch). Portal-only — no admin, no `request access` form, no notifications. Archived, draft, and guest-only projects do not appear. Inactivity reuses the existing admin `SessionMonitor` UX. Localized in en/nl/de.
- **Custom favicon upload** ([#66](https://github.com/MansiVisuals/ViTransfer/issues/66)) — admins can upload a custom favicon (SVG, PNG, or ICO, up to 100 KB) from Settings → Branding, mirroring the existing logo upload flow. SVGs are sanitized via DOMPurify with the same strict allow-list as the logo; PNG and ICO files are verified by magic bytes. When set, the custom favicon overrides the default browser-tab icons across the entire app (admin, share pages, login). Served at `/api/branding/favicon` with a 5-minute cache. Localized in en/nl/de.
- **Loop playback toggle on the video player** ([#68](https://github.com/MansiVisuals/ViTransfer/issues/68)) — clients can now enable continuous loop playback on the preview player. The toggle sits between volume and fullscreen on the controls bar; the icon highlights when active, and `aria-pressed` reflects the state for assistive technology. Localized in en/nl/de.
- **Searchable filters and saved views on the Projects Dashboard** ([#70](https://github.com/MansiVisuals/ViTransfer/issues/70)) — replaced the single status filter with a richer toolbar. Adds a search box that matches case-insensitive substrings across project title, description, company name, and recipient name/email; multi-select filters for Status, Client (typeahead from `ClientCompany` records and free-text `companyName`, grouped under one canonical key per client), Year (from `createdAt`), and Due Date (Overdue / This week / This month / Later / No due date, with "Overdue" suppressed for `APPROVED`/`ARCHIVED`/`SHARE_ONLY` to match existing UI semantics); active filters render as removable chips with a "Clear all" action; a sort menu replaces the cycle button with seven named options (Recently updated, Recently created, Oldest, Due soonest, Title A–Z/Z–A, Status priority); and a saved-views row for one-click recall of common filter combinations. Filter state mirrors to URL search params (`?q=&status=&client=&year=&due=&sort=`) so dashboard URLs are shareable, and persists to localStorage so reopening the page restores the last view. Saved views are persisted per-user via a new `AdminSavedView` table and `/api/admin/saved-views` endpoints (auth + rate-limit pattern, max 50 per user, ownership-scoped delete via `deleteMany({ id, userId })` to prevent cross-user IDOR), so they survive browser data clears and follow the user across devices. The analytics card now scopes its totals to the current filter. `/api/projects` now also returns `clientCompanyId`, `clientCompany.name`, and recipient `email` so the search and Client filter resolve correctly. Behavior change: archived projects are no longer hidden by default — empty filters show everything; users who want the old behavior can save an "Active" view excluding `ARCHIVED`. Localized in en/nl/de.

### Fixed
- **"Video record not found" when retrying a failed upload** — if a previous upload attempt failed and left a stale video record reference in local storage, retrying the same file would immediately error with "Video record not found" (or "Video is not in UPLOADING state"). Both the S3 multipart and TUS upload paths now detect stale resume metadata on these specific backend errors, clear the stale state, and silently retry once — creating a new video record and restarting the upload without any user action required.
- **Cross-project IDOR on recipient PATCH/DELETE** — `/api/projects/[id]/recipients/[rid]` allowed an admin acting through Project A's URL to edit or delete a recipient that belonged to Project B. Root cause: the lookup keyed only on `rid`, ignoring the `projectId` URL segment. Fixed by scoping the lookup to `where: { id: rid, projectId }` and returning 404 on mismatch.
- **Race conditions on default-row creation in five admin endpoints** — `findUnique → create` patterns in `clients/backfill`, `clients/[id]` PATCH (duplicate-name check), `calendar/token`, `settings`, and `settings/security` could race two concurrent requests into duplicate creates. Root cause: a check-then-act pattern with no atomicity. Fixed by converting to `prisma.upsert` or by relying on the unique constraint with a P2002 catch.
- **Video approval was not atomic** — approving v2 of a video issued an `updateMany` to unapprove other versions and a separate `update` to approve the target. Root cause: two sequential writes with no transaction; concurrent approvals could leave both versions approved. Fixed by wrapping both writes in `prisma.$transaction([...])`.
- **Possible infinite recursion in share-page guest authentication** — `SharePageClient.loadProject()` recursively invoked itself after a successful guest-token auth. Root cause: the recursive call captured a stale `shareToken` value (still `null`) from its closure and re-entered the same 401 → guest-auth path. Fixed by removing the recursive call — `setShareToken` already triggers a `useEffect` re-run via its dependency array.
- **Email-enumeration timing oracle on OTP verification** — `/api/share/[token]/verify-otp` returned a generic error to non-recipients but ran a Redis OTP lookup for valid recipients, leaking "is this email registered?" via a measurable wall-clock timing difference. Root cause: a recipient pre-check that early-returned before reaching the OTP comparison. Fixed by removing the pre-check; `verifyOTP` returns the same generic error with no side effects when no OTP exists for the `(email, project)` pair, so both branches now have identical timing.
- **Push subscription list could return HTTP 500 if any stored endpoint was malformed** — calling GET on the push subscription list crashed the response when one row had an invalid endpoint URL. Root cause: `new URL(sub.endpoint).origin` was called inside a `.map` with no per-row error handling, so a single bad row poisoned the whole response. Fixed by wrapping the URL parse per row with an empty-origin fallback.
- **Asset and video downloads showed a multi-second delay before the browser's save dialog appeared** — clicking download produced a noticeable pause before the native save prompt opened, particularly when the analytics path was slow. Root cause: the streaming content endpoints awaited a `trackVideoAccess` analytics call before emitting response headers. Fixed by changing the call to fire-and-forget so headers and the first response bytes are sent immediately.
- **Admin downloads of client-uploaded files buffered the entire file in the browser tab before saving** — a multi-GB upload sat in browser memory before the save dialog appeared, making large downloads feel like the browser was downloading the file twice. Root cause: the admin "Project Uploads" block fetched the response, called `response.blob()`, created an `URL.createObjectURL`, and only then triggered a download click. Fixed by adding a single-use download token endpoint (`/api/projects/[id]/project-uploads/[uploadId]/download-token`, IP+UA-bound, 5-minute TTL); the admin browser now navigates directly via `<a href download>`, triggering the native save dialog and a streamed download immediately. The existing GET endpoint accepts both bearer-admin and `?dlt=<token>` authentication.
- **Resumable-download clients (curl, wget, browser download managers) saw severely throttled FS-mode throughput** — when the client sent `Range: bytes=0-` to start a download, the server returned only 16 MiB per request, forcing hundreds of sequential round-trips through Prisma per file. Root cause: a defensive chunk cap intended for video-player range requests was applied to all range requests, including downloads. Fixed by introducing `parseDownloadRangeHeader`, which honors open-ended ranges as "rest of file" and explicit `bytes=A-B` ranges verbatim. The original cap remains on video-player range requests, where small chunks keep seek operations responsive.
- **Clean install on `postgres:18-alpine` failed to start with "unused mount/volume" error** — recent builds of the `postgres:18-alpine` image enforce the new PG 18+ data layout and refuse to initialize a cluster directly at `/var/lib/postgresql/data`, which is the mount target shipped in all three compose files. Fresh installs hit this on first `docker compose up`. Root cause: PG 18 expects data in a major-version subdirectory of the mount, not at the mount root. Fixed by setting `PGDATA=/var/lib/postgresql/data/pgdata` in `docker-compose.yml`, `docker-compose.unraid.yml`, and `docker-compose.truenas.yml`, so the cluster initializes inside a subdirectory of the existing mount. Existing PG 17 or PG 18 installs are unaffected as long as the volume isn't re-pointed; users wishing to migrate to PG 18 must follow their own upgrade procedure.

### Changed
- **"Download All Videos" on the share page triggers individual per-file downloads instead of a server-generated ZIP** — previously, clicking "Download All" streamed all approved videos into a single ZIP archive on the server before delivering it to the browser, which was slow and resource-intensive for large files. The share page now requests a secure per-video download token for each approved video and triggers individual downloads sequentially with a short stagger, bypassing all server-side compression. The bulk ZIP endpoint has been removed. The `archiver` dependency remains in use for per-video asset ZIP downloads.
- **Asset bulk-download ZIP no longer compresses already-compressed bytes** — `archiver` was running zlib level 6 over MP4/MOV/etc., consuming CPU for no size reduction and bottlenecking the stream. Switched to `store: true` (concatenate-with-headers, no compression). Typically 2–4× faster bulk-download throughput on multi-GB archives.
- **Worker-side S3 multipart upload is now parallel** — `s3UploadFileMultipart` (used when the worker uploads transcoded videos back to S3) previously uploaded parts sequentially. Now uses a worker pool with configurable concurrency (env `S3_SERVER_MULTIPART_CONCURRENCY`, default 4, range 1–16). Significantly faster transcoded-output uploads on fast LAN/MinIO links.
- **Worker-side S3 upload no longer buffers the entire file in memory** — `s3UploadFile` previously called `streamToBuffer()` on streams of unknown size, holding the entire transcoded output in heap before upload. Now streams chunk-by-chunk into multipart parts; memory use is bounded to roughly `partSize × concurrency` (~100 MB) regardless of file size.
- **Browser S3 multipart upload uses a worker pool** — previously processed parts in batches of 4 via `Promise.all`, where one slow part idled the other three until the batch finished. Replaced with a shared queue feeding N concurrent workers; finished parts immediately pull the next item, eliminating straggler-blocked-batch behavior.
- **Continuous byte-level upload progress in the browser** — switched from `fetch` to `XMLHttpRequest` for each S3 part PUT, exposing `xhr.upload.onprogress` for real-time progress reporting. Progress bars no longer jump in 25 MiB increments.
- **Per-part retry on transient S3 errors** — a single 5xx response from MinIO/R2/S3 no longer fails the entire multi-GB upload. Each part PUT retries up to 3 times with 0.5 s / 1.5 s / 4.5 s exponential backoff.
- **Presigned part-URL TTL raised from 1 hour to 6 hours** — slow-link uploads of large files (for example 50 GB at 50 Mbps takes roughly 2 hours) no longer fail mid-upload with expired URLs on the trailing parts.
- **TUS upload finish uses `fs.rename` instead of a streaming copy in FS mode** — completed TUS uploads were previously re-streamed from `/tmp/vitransfer-tus-uploads/...` into `STORAGE_ROOT/...` via Node's `pipeline`, doubling disk I/O on every upload. The new `moveFile` helper uses `fs.rename` (O(1) when both paths share a filesystem — the typical Docker setup) with an `EXDEV` fallback to streaming copy. Cuts post-upload disk I/O in half on the TUS path.
- **Backpressure-aware streaming for FS downloads** — `createWebReadableStream` previously enqueued every Node ReadStream chunk into a Web ReadableStream with no flow control, allowing a slow client to buffer the entire file in server RAM. Replaced with a manual `ReadableStream` that pauses the underlying Node `ReadStream` when `controller.desiredSize <= 0` and resumes it on `pull`. Keeps memory bounded without the per-chunk overhead of byte-stream adapters, which had measurably reduced throughput through Cloudflare tunnels.
- **Default FS streaming buffer raised from 4 MiB to 16 MiB** (`TRANSFER_STREAM_HWM_MB`) — the previous default was conservative for any modern server. 16 MiB remains memory-safe (per-active-transfer use is bounded to roughly this value) and improves single-stream throughput on dedicated hardware. Override range is now 1–64 MiB; tuning guidance is documented in the [Configuration wiki](docs/wiki/Configuration.md#transfer-tuning).

### Security
- **Cloud-metadata SSRF deny-list on outbound notification URLs** — admin-configurable Gotify `baseUrl` and NTFY `serverUrl` are now validated against a deny-list covering `169.254.169.254`, `metadata.google.internal`, `metadata.goog`, `metadata`, and the IPv6 metadata range (`fd00:ec2::254`). This prevents an admin (or a compromised admin account) from configuring a notification destination that targets a cloud instance-metadata service. Private LAN destinations (`192.168.x`, `10.x`, etc.) remain allowed by design, since many self-hosters legitimately run their notification provider on the same private network.
- **Missing rate limits added to six admin routes** — `/api/security/blocklist/domains` (GET/POST/DELETE), `/api/security/blocklist/ips` (GET/POST/DELETE), `/api/settings/test-email` (POST, 5/min — prevents the test-email endpoint being used as an email-bomb relay), `/api/settings/logo` (POST/DELETE), and `/api/security/events` (DELETE) now follow the standard auth → rate-limit → business-logic pattern.
- **Per-IP rate limit added to `/api/share/[token]/send-otp`** — the existing per-`(email, project)` `checkOTPRateLimit` only counts requests that pass recipient verification. Root cause: an attacker spamming the endpoint with random non-recipient emails never tripped that limit, while every attempt still triggered an "unauthorized OTP request" admin notification, allowing the endpoint to be used as a notification-flood vector. Fixed by adding a 10-per-15-minutes IP-based cap that catches all requests, including non-recipients.
- **Cryptographic randomness for client-side upload IDs** — replaced `Math.random()` fallbacks in `CommentAttachmentButton` and `useAssetUploadQueue` with `crypto.randomUUID()`. Eliminates predictable upload-ID collision risk in long-lived browser sessions.
