...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const isPhotoProject = type === 'PHOTO'
    ...
    const project = await prisma.$transaction(async (tx) => {
      const newProject = await tx.project.create({
        data: {
          ...
          enableRevisions: isShareOnly || isPhotoProject ? false : (enableRevisions || false),
          maxRevisions: isShareOnly || isPhotoProject ? 0 : (enableRevisions ? (maxRevisions || 3) : 0),
          restrictCommentsToLatestVersion: isShareOnly || isPhotoProject ? false : (restrictCommentsToLatestVersion || false),
          status: isShareOnly ? 'SHARE_ONLY' : 'IN_REVIEW',
          hideFeedback: isShareOnly ? true : false,
          approvedAt: isShareOnly ? new Date() : null,
          previewResolution: isPhotoProject ? '1080p' : (settings?.defaultPreviewResolution || '720p'),
          skipTranscoding: isPhotoProject ? true : (settings?.defaultSkipTranscoding ?? false),
          watermarkEnabled: isPhotoProject ? false : (settings?.defaultWatermarkEnabled ?? true),
          watermarkText: isPhotoProject ? null : (settings?.defaultWatermarkText || null),
          watermarkPositions: isPhotoProject ? 'center' : (settings?.defaultWatermarkPositions || 'center'),
          ...
          usePreviewForApprovedPlayback: isPhotoProject ? false : (settings?.defaultUsePreviewForApprovedPlayback ?? false),
          ...
          applyPreviewLut: isPhotoProject ? false : (settings?.defaultApplyPreviewLut ?? true),
